class MainController < ApplicationController
  ...
  skip_before_action :authenticate, only: %i[new create index]
  before_action :set_student, only: %i[edit students]
  before_action :set_company, only: %i[edit companies]
  before_action :only_students, only: %i[all_companies choosed_companies waiting_approval]
  before_action :only_companies, only: %i[all_applications pending_confirmations approved]
  ...
  def update
    if @_current_user.company
      ...
      if company
        status = company.update(pars_params_company) ? 'ok' : company.errors.full_messages
      else
        company = Company.new(pars_params_company)
        status = company.save ? 'ok' : company.errors.full_messages
      end

    else
      student = Student.find_by_user_id(session[:current_user_id])
      if student
        status = student.update(pars_params_student) ? 'ok' : student.errors.full_messages
      else
        student = Student.new(pars_params_student)
        status = student.save ? 'ok' : student.errors.full_messages
      end
    end
  ...
  def accept_offer
    ...
    respond_to do |f|
      f.json do
        render json: if offer
                       { success: (offer.update(comp_agree: true) ? true : false), error: offer.errors.full_messages }
                     else
                       { success: false, error: 'Unfound. Try to reload the page' }
                     end
      end
    end
  end
  ...
  def cancel_acceptance
    ...
    respond_to do |f|
      f.json do
        render json: if offer
                       { success: (offer.update(comp_agree: false) ? true : false), error: offer.errors.full_messages }
                     else
                       { success: false, error: 'Unfound. Try to reload the page' }
                     end
      end
  ...
  def cancel_offer
    ...
    respond_to do |f|
      f.json do
        render json: if offer
                       { success: (offer.destroy ? true : false), error: offer.errors.full_messages }
                     else
                       { success: false, error: 'Unfound. Try to reload the page' }
                     end
      end
  ...
  # send student's file
  def get_file
    file_path = "#{Rails.root}/public/uploads/#{params[:id]}-example.pdf"
  ...
  def only_companies
    redirect_to main_companies_path if !@_current_user.company || !set_company
  end
  ...
  def pars_params
    ...
    @company = !(params[:company] == 'student')
  end
  ...
  def upload
    ...
    stud_id = set_student ? set_student.id.to_s : params[:id]
    File.open(Rails.root.join('public', 'uploads', stud_id + '-example.pdf'), 'wb') do |file|
